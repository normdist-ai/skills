...
用法：
    # 基础用法（使用最佳默认参数）
    python3 selfie-pure.py

    # 测试特定表情
    python3 selfie-pure.py --expression "big grin, wide toothy smile, laughing joyfully, showing teeth"

    # 详细模式查看参数
    python3 selfie-pure.py --expression "big grin" --verbose
    ...
    for node_id, node in workflow.get("prompt", {}).items():
        if node.get("class_type") == "CLIPTextEncode" and "text" in node.get("inputs", {}):
            ...
            if is_positive:
                ...
                # 反向提示词处理 - 添加负面嵌入和强力多人脸约束
                # 负面嵌入（文本嵌入）
                negative_embeddings = [
                    'EasyNegative',
                    '(ng_deepnegative_v1_75t:1.0)',
                    '(badhandv4:1.2)',
                    'bad_prompt_version2'
                ]
