...
用法：
    # 基础用法（使用最佳默认参数）
    python3 selfie-nude.py

    # 测试特定表情
    python3 selfie-nude.py --expression "big grin, wide toothy smile, laughing joyfully, showing teeth"

    # 详细模式查看参数
    python3 selfie-nude.py --expression "big grin" --verbose
...
DEFAULT_EXPRESSION_WEIGHT = 1.2
WEIGHT_SOLO = 1.5
WEIGHT_NSFW = 1.8
WEIGHT_MULTIPLE_PEOPLE = 2.0
WEIGHT_MULTIPLE_FACES = 2.0
WEIGHT_TWO_THREE_PEOPLE = 1.9
WEIGHT_EXTRA_CLONED_FACES = 1.8
WEIGHT_NEGATIVE_EMBEDDING_DEEP = 1.0
WEIGHT_NEGATIVE_EMBEDDING_HAND = 1.2

FACE_FILE_EXTENSIONS = ["*.png", "*.jpg", "*.jpeg"]

WEATHER_CODES_SNOW = {226, 230, 326, 329, 332, 335, 338, 368, 371, 374, 377, 392, 395}
WEATHER_CODES_RAIN = {176, 200, 263, 266, 293, 296, 299, 302, 305, 308, 311, 314, 317, 320, 353, 356, 359, 386, 389}
WEATHER_CODES_CLOUDY = {116, 119, 122, 143, 248, 260}
WEATHER_API_TIMEOUT = 5
WEATHER_API_RETRY = 2
WEATHER_API_RETRY_DELAY = 1
...
def get_random_face_reference() -> str:
    ...
    face_files = [f for ext in FACE_FILE_EXTENSIONS for f in FACES_DIR.glob(ext)]
    if not face_files:
...
def get_weather():
    for attempt in range(WEATHER_API_RETRY):
        try:
            url = "https://wttr.in/Shanghai?format=j1"
            try:
                import requests
                resp = requests.get(url, headers={"User-Agent": "curl/7.68.0"}, timeout=WEATHER_API_TIMEOUT)
                resp.raise_for_status()
                data = resp.json()
            except ImportError:
                import urllib.request
                req = urllib.request.Request(url, headers={"User-Agent": "curl/7.68.0"})
                with urllib.request.urlopen(req, timeout=WEATHER_API_TIMEOUT) as r:
            ...
            if code in WEATHER_CODES_SNOW:
                return "snowy", temp, desc
            if code in WEATHER_CODES_RAIN:
                return "rainy", temp, desc
            if code in WEATHER_CODES_CLOUDY:
                return "cloudy", temp, desc
            return "sunny", temp, desc
        except Exception as e:
            if attempt < WEATHER_API_RETRY - 1:
                print(f"[WARN] 获取天气失败，重试中 ({attempt + 1}/{WEATHER_API_RETRY}): {e}", file=sys.stderr)
                time.sleep(WEATHER_API_RETRY_DELAY)
            else:
    ...
    for node_id, node in workflow.get("prompt", {}).items():
        if node.get("class_type") == "CLIPTextEncode" and "text" in node.get("inputs", {}):
            ...
            if is_positive:
                ...
                negative_embeddings = [
                    'EasyNegative',
                    f'(ng_deepnegative_v1_75t:{WEIGHT_NEGATIVE_EMBEDDING_DEEP})',
                    f'(badhandv4:{WEIGHT_NEGATIVE_EMBEDDING_HAND})',
                    'bad_prompt_version2'
                ]
                
                negative_constraints = [
                    f'(multiple people:{WEIGHT_MULTIPLE_PEOPLE})',
                    f'(multiple faces:{WEIGHT_MULTIPLE_FACES})',
                    f'(two people:{WEIGHT_TWO_THREE_PEOPLE})',
                    f'(three people:{WEIGHT_TWO_THREE_PEOPLE})',
                    f'(group:{WEIGHT_TWO_THREE_PEOPLE})',
                    f'(crowd:{WEIGHT_TWO_THREE_PEOPLE})',
                    f'(extra faces:{WEIGHT_EXTRA_CLONED_FACES})',
                    f'(duplicate face:{WEIGHT_EXTRA_CLONED_FACES})',
                    f'(cloned face:{WEIGHT_EXTRA_CLONED_FACES})',
                    f'(2girls:{WEIGHT_EXTRA_CLONED_FACES})',
                    f'(3girls:{WEIGHT_EXTRA_CLONED_FACES})',
                    f'(multiple girls:{WEIGHT_EXTRA_CLONED_FACES})'
                ]
                ...
                if added_embeddings > 0:
                    print(f"[DEBUG] 节点{node_id} - 添加 {added_embeddings} 个负面嵌入", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--seed", "-s", type=int, default=DEFAULT_SEED, help=f"指定种子（默认: {DEFAULT_SEED}）")
    ...
    if args.smile_level:
        ...
        expression_weight = DEFAULT_EXPRESSION_WEIGHT
