...
"""
韩梅梅自拍生成脚本 — cron 独立版
根据 季节+时段+天气+日常生活场景 生成自拍照。

韩梅梅的一天（交大计算机系博士生）：
  早起 → 食堂/买早餐 → 实验室/图书馆 → 午饭 → 下午继续 → 晚饭散步 → 回宿舍
...
特性：
    - 读取 faceid.json 作为基础工作流
    - 动态构建完整提示词（穿搭、表情、光线、背景）
    - 生成新的工作流文件
    - 使用 faces 文件夹中的照片作为脸部修正参考
"""
...
def update_prompt(workflow, clothing=None, expression=None, lighting=None, background=None):
    """更新工作流中的提示词，构建完整描述"""
    for node_id, node in workflow.get("prompt", {}).items():
        if node.get("class_type") == "CLIPTextEncode" and "text" in node.get("inputs", {}):
            text = node["inputs"]["text"]
            
            print(f"[DEBUG] 原始提示词: {text[:200]}...", file=sys.stderr)
            
            if clothing:
                text = re.sub(r'wearing [^,]+', f'wearing {clothing}', text)
                print(f"[DEBUG] 替换穿搭: {clothing}", file=sys.stderr)
            
            if expression:
                text = re.sub(r'playful expression, cute smile, twinkling eyes', expression, text)
                text = re.sub(r'(gentle|natural|happy|elegant|relaxed) smile', expression, text, flags=re.IGNORECASE)
                if 'smile' not in text.lower() and 'expression' not in text.lower():
                    text = text.replace('beautiful young Chinese woman', f'beautiful young Chinese woman, {expression}')
                print(f"[DEBUG] 替换表情: {expression}", file=sys.stderr)
            ...
            if lighting:
                ...
                print(f"[DEBUG] 替换光线: {lighting}", file=sys.stderr)
            
            if background:
                text = text + f', {background}'
                print(f"[DEBUG] 添加背景: {background}", file=sys.stderr)
            
            print(f"[DEBUG] 最终提示词: {text[:200]}...", file=sys.stderr)
...
def main():
    now = datetime.now()
    print(f"[INFO] === 韩梅梅自拍生成 {now.strftime('%Y-%m-%d %H:%M:%S')} ===", file=sys.stderr)

    season = get_season()
    period = get_period()
    weather, temp, weather_desc = get_weather()
    print(f"[INFO] 季节={season}, 时段={period}, 天气={weather}({temp}°C,{weather_desc})", file=sys.stderr)
    ...
    print(f"[INFO] 场景={scene_config}", file=sys.stderr)
    ...
    print(f"[INFO] 表情={expression}", file=sys.stderr)
    print(f"[INFO] 背景={background}", file=sys.stderr)
    print(f"[INFO] 随机种子={seed}", file=sys.stderr)
    ...
    workflow = update_prompt(workflow, clothing=clothing, expression=expression, lighting=lighting, background=background)

    for node_id, node in workflow.get("prompt", {}).items():
        if "seed" in node.get("inputs", {}):
            node["inputs"]["seed"] = seed
            print(f"[DEBUG] 设置种子: 节点{node_id} = {seed}", file=sys.stderr)
    ...
    if face_reference:
        ...
        for node_id, node in workflow.get("prompt", {}).items():
            if node.get("class_type") == "LoadImage":
                ...
                print(f"[DEBUG] 设置脸部参考: 节点{node_id} = {face_filename}", file=sys.stderr)
    ...
    try:
        ...
        new_filename = f"HMM-{now.strftime('%Y%m%d-%H%M%S')}-{seed}.png"
