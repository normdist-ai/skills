...
改动：
    - 提示词从 prompts/ 目录的文本文件读取，不再硬编码
    - base.txt 提供正向/负向基础提示词
    - 各场景文件（boulevard.txt, cafe.txt 等）提供差异覆盖
    - 保留 FaceID 面部一致性、环境感知、天气修正
    - 网上看到好的提示词，直接粘贴到对应文件即可

用法：
    python3 selfie-v3.py                    # 自动场景+时段
    python3 selfie-v3.py --scene cafe       # 指定场景
    python3 selfie-v3.py --prompt-file prompts/my_style.txt  # 指定提示词文件
    python3 selfie-v3.py --nude             # nude 模式
    python3 selfie-v3.py --smile-level 4    # 笑容等级

输出（stdout）：
    成功：一行消息文字 + MEDIA:路径
    失败：错误信息到 stderr，stdout 为空
"""
    ...
    解析提示词文件，返回 dict:
      {
        ...
        "negative": "负向提示词文本",
        "sections": {
            "spring.morning": {"outfit": "...", "expression": "...", "lighting": "...", "background": "..."},
            ...
        }
      }
    """
    ...
    for line in lines:
        ...
        if m:
            ...
            if current_section in ("positive", "negative"):
                # base.txt 的 [positive] / [negative] 段内可能有多行
        ...
        # 段内无 key 的纯文本（base.txt 正向/负向多行拼接）
        if current_section in ("positive", "negative"):
            if result[current_section]:
...
def load_scene_prompts(scene: str, season: str, period: str) -> dict:
    """
    加载提示词：base + 场景覆盖
    返回 {"positive": ..., "negative": ..., "outfit": ..., "expression": ..., "lighting": ..., "background": ...}
    """
    # 1. 读 base.txt
...
# ══════════════════════════════════════════════════════════════
#  工作流更新（核心改动：直接写提示词文本，不再正则替换）
# ══════════════════════════════════════════════════════════════

def get_time_description(period: str, hour: int) -> str:
    """根据时段、具体小时和日期生成时间提示词，影响光线氛围"""
...
def update_workflow_prompts(workflow, positive_text, negative_text):
    """直接将提示词文本写入工作流的 CLIPTextEncode 节点"""
    for node_id, node in workflow.get("prompt", {}).items():
        if node.get("class_type") == "CLIPTextEncode" and "text" in node.get("inputs", {}):
            ...
            if is_positive:
                ...
                print(f"[DEBUG] 节点{node_id} - 正向提示词: {positive_text[:150]}...", file=sys.stderr)
            else:
                node["inputs"]["text"] = negative_text
                print(f"[DEBUG] 节点{node_id} - 负向提示词: {negative_text[:150]}...", file=sys.stderr)


def update_workflow_params(workflow, faceid_weight=0.80, cfg=4.0, start_at=0.0, faceid_end_at=1.0, faceid_weight_0=1.0):
    """更新工作流中的 KSampler 和 IPAdapterFaceID 参数"""
    for node_id, node in workflow.get("prompt", {}).items():
        ...
        if class_type == "KSampler" and "cfg" in inputs:
            inputs["cfg"] = cfg
            print(f"[DEBUG] 节点{node_id} (KSampler) - CFG: {cfg}", file=sys.stderr)

        elif class_type == "IPAdapterFaceID":
            if "weight" in inputs:
                inputs["weight"] = faceid_weight_0
                print(f"[DEBUG] 节点{node_id} (IPAdapterFaceID) - weight: {faceid_weight_0}", file=sys.stderr)
            if "weight_faceidv2" in inputs:
                inputs["weight_faceidv2"] = faceid_weight
                print(f"[DEBUG] 节点{node_id} (IPAdapterFaceID) - weight_faceidv2: {faceid_weight}", file=sys.stderr)
            if "start_at" in inputs:
                inputs["start_at"] = start_at
                print(f"[DEBUG] 节点{node_id} (IPAdapterFaceID) - start_at: {start_at}", file=sys.stderr)
            if "end_at" in inputs:
                inputs["end_at"] = faceid_end_at
                print(f"[DEBUG] 节点{node_id} (IPAdapterFaceID) - end_at: {faceid_end_at}", file=sys.stderr)
    return workflow
...
def main():
    parser = argparse.ArgumentParser(description="韩梅梅自拍生成 v3 — 提示词外置版")
    ...
    parser.add_argument("--seed", "-s", type=int, help="指定种子")
    ...
    parser.add_argument("--prompt-file", "-p", type=str, help="指定提示词文件（覆盖自动场景选择）")
    parser.add_argument("--verbose", "-v", action="store_true", help="详细输出")
    args = parser.parse_args()

    now = datetime.now()
    print(f"[INFO] === 韩梅梅自拍 v3 {now.strftime('%Y-%m-%d %H:%M:%S')} ===", file=sys.stderr)
    ...
    if args.smile_level:
        ...
        if expression_weight is None:
            expression_weight = smile_config["weight"]
        print(f"[INFO] 默认笑容等级: 3 - {smile_config['description']}", file=sys.stderr)
    ...
    # ── 确定场景 ──
    if args.prompt_file:
        # 用户直接指定提示词文件（可以不是场景文件，比如从网上复制的）
        prompt_file = args.prompt_file
        scene_config = Path(prompt_file).stem  # 用文件名作为场景名
        lighting = ""
        print(f"[INFO] 使用指定提示词文件: {prompt_file}", file=sys.stderr)
    else:
        if args.scene:
            # 用户指定场景，lighting 从场景文件读取（稍后加载）
    ...
    positive_text = build_final_positive(
        ...
        background if 'background' in dir() else "",
        args.nude, time_description
    )
    ...
    if face_reference:
        ...
        for node_id, node in workflow.get("prompt", {}).items():
            if node.get("class_type") == "LoadImage":
                ...
                print(f"[DEBUG] 节点{node_id} - 脸部参考: {face_filename}", file=sys.stderr)
    ...
    try:
        ...
        new_filename = f"HMM-v3-{now.strftime('%Y%m%d-%H%M%S')}-{seed}.png"
