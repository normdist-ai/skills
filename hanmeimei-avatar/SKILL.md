...
description: |
  韩梅梅形象生成技能 — 为 Hermes Agent 开发，通过飞书发送 AI 写真照片给用户。
  用 ComfyUI 生成韩梅梅的 AI 写真照片和未来的语音视频。
  保持面部一致性、写实风格、超白皙肌肤。
  当用户说"给我发张照片"、"出张图"、"看看你"、"你的照片"、"勾搭我"时触发。
  当用户说"涩图"、"nude"、"裸"时使用 --nude 模式。
  调用 comfyui 技能执行实际生成。
  主通道：Hermes → feishu_sender.py → 飞书三气泡。辅助通道：Trae IDE → trae_sender.py。
metadata:
  author: 韩梅梅
  version: "5.2.0"
...
1. 根据用户提示，或者定时任务，结合生活场景，生成提示词。
2. 根据提示词和配置信息，生成 ComfyUI 工作流文件。
3. 调用 comfyui 技能执行生成图片或视频任务。
4. **主通道**：Hermes Agent 通过 `feishu_sender.py` 将图片/消息发送到飞书。
5. **辅助通道**：Trae IDE 通过 `trae_sender.py` 渲染 Markdown 消息。
...
- **v5 默认随机选择 1-5 级笑容** — v5 在不指定 --smile-level 时通过 `random.choice(list(SMILE_LEVELS.keys()))` 随机选择笑容等级，增加表情自然度和出图多样性。如需固定等级，使用 `--smile-level N` 参数
...
- **手部提示词已从 base.txt 移除，让模型自由发挥** — base.txt 注释明确"完全移除所有手部约束，让模型自己发挥"。后续通过 HandRefiner 流程（HandRefiner+control_sd15_inpaint_depth_hand）改进手部质量，张阳 2026-05-23 确认效果比 v4 好
...
- **推荐使用 v5（selfie-v5.py）进行自拍** — v5 是当前 cron 生产版和推荐入口，v3/v2 已退役。支持 good-seeds.txt 种子池、--scene、--nude、--smile-level 等全部参数
...
- **图片编号规则** — 文件名格式：`HMM-YYYYMMDDHHMMSS-{seed}.png`（时间戳+种子号，无连字符分隔日期和时间）。v5 代码：`f"{filename_prefix}-{photo_id}.png"`，photo_id = `now.strftime('%Y%m%d%H%M%S') + "-" + str(seed)`。
...
- **v5 防裸体依赖场景级 [negative] section** — v5 的 build_final_positive 不再拼接 `bra, round neck t-shirt`。防裸体策略改为：base.txt 无穿搭约束 → 场景 outfit 提供衣物描述 → 场景级 [negative] section 提供 `(nsfw:1.8),(nude:1.5)` 兜底
...
- **park.txt 和 travel.txt 已补齐（2026-05-24）** — 之前 ROUTINES 引用了 park/travel 场景但没有对应提示词文件，导致无 outfit → 裸体风险。已创建完整的 4季×6时段 场景文件。新增场景时必须同步创建 prompts/{scene}.txt，不能只在 ROUTINES 里加条目。cron 和手动出图统一使用 v5。selfie-v4.py 是种子池版（备选）。selfie.py（v0）是旧生产版，已退役。
- **HandRefiner 手部修复流程已确认优于 v4 提示词方案** — HandRefiner+control_sd15_inpaint_depth_hand 手部修复流程，张阳 2026-05-23 确认效果比 v4 的手部提示词策略更好。详见 [references/hand-fix-research.md](references/hand-fix-research.md)
- **批量出图反馈** — 种子号和图要一起发，便于用户识别和挑选好种子加入 good-seeds.txt。
- **v5 防裸体配方（场景级 negative 兜底）** — v5 已将三层防御拆解重构：
  1. base.txt：无穿搭约束，让场景自由发挥
  2. 场景 outfit 提供具体衣物描述（如 `white blouse, long skirt, bra`），不能只写 `casual outfit`
  3. 场景级 [negative] section 提供 `(nsfw:1.8),(nude:1.5)` 兜底，nude 模式单独控制
  **肩部和乳沟可以露**，不加 `(bare shoulders)` 和 `(cleavage)` 负面词。场景 outfit 必须具体，太模糊容易跑偏出裸体
...
> **主通道**：Hermes Agent → feishu_sender.py → 飞书（用户日常交互平台）  
> **辅助通道**：Trae IDE → trae_sender.py → 渲染 Markdown 消息

### 架构（v5 — 统一输出，双平台发送）

```
selfie-v5.py（生图，stdout 输出结构化文本）
    ├─→ feishu_sender.py（主通道 — Hermes cron → 飞书三气泡）
    └─→ trae_sender.py（辅助通道 — Trae IDE → Markdown 渲染）
```

- `selfie-v5.py`：纯生图，stdout 输出统一格式文本
- `feishu_sender.py`（主）：解析输出，分三个气泡调 lark_oapi 发飞书（①图片→②信息→③想法）
- `trae_sender.py`（辅）：解析输出，渲染为 Markdown 消息

### 主通道：飞书三气泡（feishu_sender.py）

**cron 自动发送：**
`selfie.sh` → `selfie-v5.py`（生图）→ `feishu_sender.py`（三气泡发送）→ stdout 空

1. 气泡1（图片）：上传到飞书并发送
2. 气泡2（描述）：时间/地点/种子（纯文本，无 emoji 和引用标记）
3. 气泡3（想法）：想法文字

**手动发送（agent 交互模式，用 send_message 工具）：**
同样三气泡格式。
...
### 辅助通道：Trae 单条消息（trae_sender.py）

渲染为 Markdown 格式，所有内容合并在一条消息中：
```
[HMM-xxx.png](file:///path/to/image.png)
---
...
| 变量 | 内容 | 示例 |
|------|------|------|
| `{image}` | Markdown 图片链接 | `[HMM-xxx.png](file:///path.png)` |
