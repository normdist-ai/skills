...
metadata:
  ...
  version_changelog: "v4.4: v4负面词对齐v0后出图质量用户确认OK；seed=-1修复；GitHub独立仓库 normdist-ai/hanmeimei-avatar 建立；good-seeds.txt扩充至4颗"
...
- **ComfyUI 拒绝 seed=-1** — `--random-seed` 不能传 -1 给 ComfyUI（会被秒拒返回空错误）。必须用 `random.randint(1, 999999999)` 生成正整数种子。v4 已修复（原代码 `return -1` 改为 `return random.randint(1, 999999999)`）
...
- **种子管理策略（v4 种子池版，学术+社区验证）** — arXiv 2405.14828（46M图实验）证明"黄金种子"跨提示词泛化性好；Reddit wonderflex 教程证明每个种子有内在"主题"（色温/构图/默认服装）。管理方式：`good-seeds.txt` 保存验证好种子 → selfie-v4.py 默认从中随机选取 → `--seed N` 手动覆盖 → `--random-seed` 探索新种子。筛选流程：跑20张随机种子 → 用户挑好的 → 追加到 good-seeds.txt（用 `--add-seed` 或手动）。详见 [references/good-seeds.md](references/good-seeds.md)
...
- **ComfyUI 不接受 seed=-1** — `--random-seed` 不能传 -1 给 KSampler，会立即返回空错误。必须用 `random.randint(1, 999999999)` 生成实际种子值，与 v0 的做法一致。
- **版本管理：v0=生产，v4=开发** — selfie.py (v0) 是定时任务用的稳定版，selfie-v4.py 是开发实验版。不混用。v0 确认效果好的提示词变更才合并到 v4。
- **ComfyUI 自带 FIFO 队列** — POST `/prompt` 自动排队串行执行，不会 OOM。cron v0 和手动 v4 同时提交只会排队等，不会冲突崩溃。但可能导致 HTTP 轮询超时报"生成失败"。
...
技能源码在 GitHub 私有仓库 **normdist-ai/hanmeimei-avatar**。

```bash
# 推送本地修改到远程
cd ~/.hermes/profiles/hanmeimei/skills/creative/hanmeimei-avatar
